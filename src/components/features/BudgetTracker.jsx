...
import { Edit, Trash2, PlusCircle, X, TrendingUp, TrendingDown, DollarSign, PieChart as PieChartIcon, Calendar, Filter } from "lucide-react";
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="budget-header bg-white rounded-3xl shadow border border-gray-100 p-8 mb-8">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            <div className="header-content">
              ...
              <p className="header-description text-gray-600 text-lg">Take control of your finances with smart expense tracking</p>
            </div>
            <div className="header-controls flex flex-col sm:flex-row gap-4">
              <div className="date-selectors flex gap-2">
                <div className="relative">
                  <Calendar className="calendar-icon absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400" size={18} />
                  <select
                    value={selectedMonth}
                    onChange={(e) => setSelectedMonth(Number(e.target.value))}
                    className="month-selector pl-10 pr-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 bg-white"
                  >
                ...
                <select
                  ...
                  className="year-selector px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 bg-white"
                >
              ...
              <button
                ...
                className="add-entry-btn group relative px-8 py-4 bg-gradient-to-r from-emerald-600 to-teal-600 text-white font-semibold rounded-2xl shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-200 flex items-center gap-3"
              >
          ...
          <div className="income-card bg-white rounded-3xl shadow border border-gray-100 p-8 relative overflow-hidden">
            <div className="card-content flex items-center gap-4">
              <div className="icon-container p-4 bg-gradient-to-br from-green-100 to-green-200 rounded-2xl">
                <TrendingUp className="income-icon text-green-600" size={32} />
              </div>
          ...
          <div className="expense-card bg-white rounded-3xl shadow border border-gray-100 p-8 relative overflow-hidden">
            <div className="card-content flex items-center gap-4">
              <div className="icon-container p-4 bg-gradient-to-br from-red-100 to-red-200 rounded-2xl">
                <TrendingDown className="expense-icon text-red-600" size={32} />
              </div>
              <div className="card-text">
          ...
          <div className="balance-card bg-white rounded-3xl shadow border border-gray-100 p-8 relative overflow-hidden">
            ...
            <div className="card-content flex items-center gap-4">
              <div className={`icon-container p-4 ${balance >= 0 ? 'bg-gradient-to-br from-blue-100 to-blue-200' : 'bg-gradient-to-br from-orange-100 to-orange-200'} rounded-2xl`}>
                <DollarSign className={balance >= 0 ? 'balance-icon text-blue-600' : 'balance-icon text-orange-600'} size={32} />
              </div>
              <div className="card-text">
        ...
        {currentMonthEntries.length === 0 ? (
          <div className="empty-state bg-white rounded-3xl shadow-xl border border-gray-100 p-12 text-center">
            ...
            <h2 className="empty-title text-2xl font-bold text-gray-800 mb-4">No Entries This Month</h2>
            ...
            <button
              onClick={() => { resetForm(); setIsModalOpen(true); }}
              className="add-first-entry-btn px-8 py-4 bg-gradient-to-r from-emerald-600 to-teal-600 text-white font-semibold rounded-2xl shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-200 flex items-center gap-3 mx-auto"
            >
              <PlusCircle size={20} />
          ...
          <div className="budget-content grid grid-cols-1 xl:grid-cols-2 gap-8">
            <div className="chart-container bg-white rounded-3xl shadow-xl border border-gray-100 p-8">
              <div className="chart-header flex items-center gap-3 mb-6">
                <PieChartIcon className="chart-icon text-purple-600" size={24} />
                <h2 className="chart-title text-2xl font-bold text-gray-800">Expense Breakdown</h2>
              </div>
              {chartData.length > 0 ? (
                <div className="chart-content space-y-6">
                  ...
                  <div className="chart-legend grid grid-cols-2 gap-2">
                    {chartData.map((item, index) => (
                      <div key={item.name} className="legend-item flex items-center gap-2">
                        ...
                        ></div>
                        <span className="legend-label text-sm text-gray-600 truncate">{item.name}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ) : (
                <div className="chart-empty flex flex-col items-center justify-center h-64 text-gray-500">
            ...
            <div className="transactions-container bg-white rounded-3xl shadow-xl border border-gray-100 p-4 md:p-8">
              <div className="transactions-header flex items-center justify-between mb-6">
                <div className="flex items-center md:gap-3 gap-1">
                  <Filter className="filter-icon text-blue-600" size={24} />
              ...
              <div className="transactions-list space-y-4 max-h-96 overflow-y-auto">
                {currentMonthEntries
                  ...
                  .map((entry, index) => (
                    <div
                      key={entry._id}
                      className="transaction-item group bg-gradient-to-r from-gray-50 to-white border border-gray-100 rounded-2xl p-4 hover:shadow-md transition-all duration-200"
                    >
                      <div className="transaction-content flex justify-between items-start">
                        <div className="transaction-details flex-1">
                          <div className="flex items-center gap-2 mb-1">
                            <div className={`type-indicator w-2 h-2 rounded-full ${entry.type === 'income' ? 'bg-green-500' : 'bg-red-500'}`}></div>
                            <h4 className="transaction-name font-semibold text-gray-800">
                              {entry.description || entry.category}
                            </h4>
                          </div>
                          <p className="transaction-category text-sm text-gray-500 mb-1">{entry.category}</p>
                          <p className="transaction-date text-xs text-gray-400">
                        ...
                        <div className="transaction-actions flex items-center gap-3">
                          <div className="transaction-amount text-right">
                            <p className={`amount-text font-bold text-lg ${entry.type === 'income' ? 'text-green-600' : 'text-red-600'}`}>
                              {entry.type === 'income' ? '+' : '-'}{formatCurrency(entry.amount)}
                            </p>
                          </div>
                          <div className="action-buttons md:opacity-0 md:group-hover:opacity-100 transition-opacity flex gap-1">
                            <button
              ...
              <form onSubmit={handleAddOrUpdateEntry} className="space-y-6">
                <div className="form-grid grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="form-group">
                    <label className="form-label block text-gray-700 font-semibold mb-2">Transaction Type *</label>
                    <div className="type-buttons grid grid-cols-2 gap-2">
                      {['income', 'expense'].map((type) => (
                        <button
                          key={type}
                          type="button"
                          onClick={() => handleFormChange({ target: { name: 'type', value: type } })}
                          className={`type-button py-3 px-4 rounded-xl font-medium transition-all duration-200 ${formData.type === type
                            ? type === 'income'
                  ...
                  <div className="form-group">
                    <label className="form-label block text-gray-700 font-semibold mb-2">Amount *</label>
                    <div className="amount-input relative">
                      <span className="currency-symbol absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-500 font-medium">$</span>
                      <input
                        type="number"
                        name="amount"
                        ...
                        placeholder="0.00"
                        className="form-input w-full pl-8 pr-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors"
                        required
                        min="0.01"
                        step="0.01"
                      />
                ...
                <div className="form-grid grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="form-group">
                    <label className="form-label block text-gray-700 font-semibold mb-2">Category *</label>
                    <select
                      name="category"
                      value={formData.category}
                      onChange={handleFormChange}
                      className="form-select w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors"
                      required
                    >
                  ...
                  <div className="form-group">
                    <label className="form-label block text-gray-700 font-semibold mb-2">Date *</label>
                    <input
                      type="date"
                      name="date"
                      value={formData.date}
                      onChange={handleFormChange}
                      className="form-input w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors"
                      required
                    />
                  </div>
                </div>
                ...
                <div className="form-group">
                  ...
                  <textarea
                    ...
                    className="form-textarea w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors resize-none"
                    rows="3"
                  ></textarea>
                </div>
                ...
                <div className="modal-actions flex gap-4 pt-4">
                  ...
                  <button
                    type="submit"
                    className="submit-btn flex-1 px-6 py-3 bg-gradient-to-r from-emerald-600 to-teal-600 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-200 disabled:opacity-50 disabled:transform-none disabled:cursor-not-allowed"
                    disabled={loading}
                  >
